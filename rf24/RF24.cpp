 ...
 */
/*
 * Modified for STM32 by Daniel Holth <dholth@fastmail.fm>
 */
...
void RF24::print_status(uint8_t status)
{
  printf_P(
      ...
      (status & _BV(MAX_RT)) ? 1 : 0, ((status >> RX_P_NO) & 0x7),
...
void RF24::print_observe_tx(uint8_t value)
{
  printf_P(PSTR("OBSERVE_TX=%02x: POLS_CNT=%x ARC_CNT=%x\r\n"), value,
      (value >> PLOS_CNT) & 0xf, (value >> ARC_CNT) & 0xf);
}
...
RF24::RF24(uint8_t _cepin, uint8_t _cspin) :
    ce_pin(_cepin), csn_pin(_cspin), wide_band(true), p_variant(false), payload_size(
        32), ack_payload_available(false), dynamic_payloads_enabled(false), pipe0_reading_address(
        0)
{
}
...
void RF24::startListening(void)
{
  ...
  stm32plus::MillisecondTimer::delay(1);
}
...
void RF24::openWritingPipe(const uint8_t *value)
{
...
void RF24::openReadingPipe(uint8_t child, const uint8_t *address)
{
  ...
  if (child <= 6)
  {
    ...
    {
      write_register(RX_ADDR_P0 + child, address, 1);
    }
...
{
  // LOCK_UNLOCK some features of the nRF24L01.
  // The + part does not have this command but keeps the FEATURE register.
  csn(LOW);
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  SPI.transfer( W_ACK_PAYLOAD | (pipe & 0x7));
  const uint8_t max_payload_size = 32;
